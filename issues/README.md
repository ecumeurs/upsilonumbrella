...
| ISS-137 | [ISS-137_20260826_auth_renewal_logout_revocation_hole.md](ISS-137_20260826_auth_renewal_logout_revocation_hole.md) | Medium | Open | Logout revoked only the presented token, not its renewal lineage; fixed 2026-09-19 via a `family_id` cascade (kept Open pending soak/atom promotion) |
...
| ISS-134 | [ISS-134_20260826_go_work_sync_rewrites_submodule_gomod.md](ISS-134_20260826_go_work_sync_rewrites_submodule_gomod.md) | Medium | In Progress | `go work sync` rewrites submodule `go.mod`/`go.sum` on every CI run — committed manifests have drifted from what the workspace resolves |
...
### Index integrity notes (2026-09-18 cleanup)
- **ISS-165** (GDPR erasure fan-out to economy) merged to `main` across
  `upsilonauth`/`upsilonplatform`/root, then marked `Resolved` and deleted.
- **ISS-106** (zombie match on engine-start failure), **ISS-116** (admin PII
  censorship) and **ISS-125** (address/birth_date optional) all had their fixes
  verified against committed code before being marked `Resolved` and deleted.
- **ISS-161** and **ISS-118** (`Ref_20260722_gdpr_export_per_game_gap.md`) had their
  `Status` field read `Resolved` and were deleted per the standing open-issues-only
  convention. Content preserved in git history.
