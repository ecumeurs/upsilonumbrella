...
|---|---|---|---|---|
| ISS-167 | [ISS-167_20260918_gdpr_erasure_inventory_purge_undecided.md](ISS-167_20260918_gdpr_erasure_inventory_purge_undecided.md) | Medium | Open | ISS-165's economy-purge fix deliberately left `player_inventory`/`inventory_transactions` untouched. The two tables don't have a symmetric fix: `inventory_transactions.transaction_type` is closed by a CHECK constraint with no purge/closure value (unlike `credit_transactions.source`, a free varchar), and `player_inventory` has no audit-row concept at all. Needs a product/legal pseudonymize-vs-erase decision per table before any code change |
| ISS-166 | [ISS-166_20260917_gdpr_atoms_understate_fail_closed_contract.md](ISS-166_20260917_gdpr_atoms_understate_fail_closed_contract.md) | Low | Open | ISS-118's post-task ATD sync found the code correct but the spec incomplete: the fail-closed unconfigured-collector case (the export's headline guarantee) is missing from all three governing atoms' incomplete-trigger enumerations, `selectGDPRCollector` is untagged, the `tactical` key-omission rule is undocumented, and `api_profile_export` has no `@spec-link` so it reports `NO_IMPL`. Blocks STABLE promotion of the orchestrator atom |
| ISS-165 | [ISS-165_20260917_gdpr_erasure_never_reaches_economy.md](ISS-165_20260917_gdpr_erasure_never_reaches_economy.md) | High | Open | `upsiloneconomy` implements an idempotent GDPR purge and exposes it internally, but nothing in the platform ever calls it — auth's delete path anonymizes/soft-deletes locally and fans out a single `account_push` job to the hub, which only mirrors `deleted_at` into `playerstats`. The hub's economy client has no purge method at all. Verified live: 13 anonymized accounts, 0 wallet and 0 `gdpr_purge` ledger rows. Credits, ledgers and inventory survive erasure (GDPR Art. 17); the mirror of ISS-118's portability gap |
| ISS-164 | [ISS-164_20260916_expectation_section_gap_across_atom_corpus.md](ISS-164_20260916_expectation_section_gap_across_atom_corpus.md) | Medium | Open | 77 atoms across eight projects have no usable `## EXPECTATION` section — 29 carry the heading with an empty body (passes a naive grep, fails `atd lint`), 48 lack it entirely. The four Phase 3–6 extracted services (auth/economy/hub/platform) are clean, so this is legacy authoring debt, not a regressing template. `upsilonapi` (ISS-162) and `upsilonaws` (ATD retired) excluded by maintainer ruling |
| ISS-163 | [ISS-163_20260916_precommit_orphan_check_ignores_excluded_types.md](ISS-163_20260916_precommit_orphan_check_ignores_excluded_types.md) | Medium | Open | The ATD pre-commit hook decides "orphan" from `layer` alone and never reads `type`, so it ignores `.atd`'s `OrphanExcludedTypes` and disagrees with `atd crawl --gaps`. `upsilonbattleui:module_frontend` is a legitimate parentless root aggregator that `atd` accepts but the hook would block, with error text advising fixes that are both wrong for that atom |
| ISS-162 | [ISS-162_20260916_upsilonapi_not_on_platform_kit.md](ISS-162_20260916_upsilonapi_not_on_platform_kit.md) | Medium | Open | `upsilonapi` predates the `upsilonplatform` extraction — no OTel, no shared observability/httpx/database plumbing, unlike hub/auth/economy. Also floats a `upsilonapi` → `upsilonbattleapi` rename once tycoon/spy/digital get their own engine bridges |
| ISS-161 | [ISS-161_20260916_hub_database_url_mismatch.md](ISS-161_20260916_hub_database_url_mismatch.md) | Medium | Resolved | `docker-compose.yaml` declared the hub's `DATABASE_URL` against the Postgres maintenance db (`postgres`) instead of the dedicated `upsilon` database `deploy/initdb` provisions for it — root cause was narrower than originally framed (a one-line compose fix, not a `start_services.sh` rewrite). Fixed 2026-09-16: hub retargeted to `upsilon?sslmode=disable`, re-migrated/reseeded, `check_services.sh` now asserts (not just reads) each service's expected database name |
...
| ISS-118 | [Ref_20260722_gdpr_export_per_game_gap.md](Ref_20260722_gdpr_export_per_game_gap.md) | Medium | Resolved | Game-agnostic accounts remodel narrows auth's GDPR export to account+registrations; per-game export fragments needed before the Phase-4 cutover leaves data portability incomplete |
