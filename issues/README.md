...
| ISS-137 | [ISS-137_20260826_auth_renewal_logout_revocation_hole.md](ISS-137_20260826_auth_renewal_logout_revocation_hole.md) | Medium | Resolved | Logout revoked only the presented token, not its renewal lineage; fixed via a `family_id` cascade and verified against a real dev database 2026-09-20 |
...
| ISS-134 | [ISS-134_20260826_go_work_sync_rewrites_submodule_gomod.md](ISS-134_20260826_go_work_sync_rewrites_submodule_gomod.md) | Medium | Resolved | `go work sync` rewrote submodule `go.mod`/`go.sum` on every CI run; manifests reconciled and committed, CI drift guard added |
